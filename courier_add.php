...
$msg=mysqli_query($conn,"INSERT INTO `user`(`fname`, `lname`, `email`, `contactno`, `country`, `state`, `district` , `city`,`warda`, `localaddress`, `localaddress1`,`zipcode`,`password`, `cpassword`, `tokan`, `status`) VALUES ('$fname','$lname','$email','$contactno','$country','$state','$district','$city','$warda','$local_ad','$localaddress1','$zip','$pass','$cpass','$tokan','courier')");
echo "$msg";
  # code...
...
$_SESSION['id']=$num['id'];
$_SESSION['localaddress1']=$num['localaddress1'];
...
if ($num['status']=='inactive') { 
  ...
  echo "<script>alert(' access refused! your account not activate yet!  please check your mail to activate your account ,thank you for being with us @ kupindetimur.com');</script>";
    ...
    <div class="col-sm-4 bg-dange container"></div>
     <div class="col-sm-4 bg-succes w3-container">
      <div class="bg-primary container w3-round-large w3-panel p-2 mt-0 mr-0 ml-0"> 
        <h5 class="form-login-heading  w3-center w3-text-white ">Welcome Login & Courier Registration System</h5></div>
...
<i class="fa fa-plus"></i> C   reate Courier Account
</button>
              </div> 
              <br> 
...
function showcountry(str) {
  ...
  xhttp.open("GET", "dd1.php?q="+str, true);
  xhttp.send();
}
</script>
...
<!--   <label class="w3-left ml-0 w3-panel"> <b>district:</b></label><input class="form-control p-4"  required placeholder="enter district **" type="text"  name="district"> -->
 ...
 <label class="w3-left ml-0 w3-panel"> <b>Courier Service Name:</b></label><select class="form-control" name="zip">
  <option>--select--</option>
   <?php 
     include("./adminstatus/blogadmin/dbc.php");
  
              // INSERT INTO `state`(`id`, `country`, `state`) VALUES ([value-1],[value-2],[value-3])
             $selectquery = "SELECT*FROM courier  order by id desc ";
            $query = mysqli_query($conn , $selectquery) or die("error");
    ...
    <option value="<?php echo $result['district'] ?>" name=""><?php echo $result['district'] ?></option>
...
<br> <label class="w3-left ml-0 w3-panel"> <b> Branch Of courier:</b></label><input class="form-control p-4"   placeholder="Courier Branch Office *" type="text"  name="localaddress1">
