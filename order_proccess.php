...
$cc=$_SESSION['id'];
            ...
            // $id=$_GET['$cc'];
            $selectquery = "SELECT*FROM `order` WHERE cid ='$cc' AND subtotal=0 and status='Pending' and cas='0' order by id desc";
...
?>
	<tr>
		...
		<td class="w3-center">   <?php  echo $result['photo']; ?>.<?php  echo $result['price']; ?>|- </td>
		<td class="w3-center">	
<input  type="number" name="pquantity" id="quantity"
 class=" onlynumber  " value="<?php echo $result['pquantity']; ?>" min="1" style="width: 90px;" >
		...
		<option type="text" name="unit"> <?php echo $result['tax']; ?></option> 	
...
<div class="container w3-hide-large">
		...
		<table class="container-fluid w3-rose responsive table-stripe w3 w3- responsive" border="0" cellpadding="10"  cellspacing="10">
...
$cc=$_SESSION['id'];
             ...
             include("./adminstatus/blogadmin/dbc.php");
            // $id=$_GET['$cc'];
            $selectquery = "SELECT*FROM `order` WHERE cid ='$cc' AND subtotal=0  and cas='0' order by id desc";
            $query = mysqli_query($conn , $selectquery) or die("error");
...
<div><h2 style="border-bottom: 2px dotted red; color: orange;">Delevery Address:<h4 style="color: green;">


      <?php echo $cname; ?></h4><span style="font-size: 20px;">
			...
			<br>
			  <div class=""><h2 style="border-bottom: 2px dotted red; color: orange;">Choose Payment Method</h4 >
          <br>
          <form action="payment_up.php" method="POST" enctype="multipart/form-data">
<div class="ml-5" id="ok">
<input style="width: 25px; height: 25px;" type="radio" name="payment" onclick="getpayment(this.value)" value="cash" >    <label style="font-weight: bold; color: green; font-size: 20px;">Cash On Delevery</label> <br>
<input style="width: 25px; height: 25px;" type="radio" name="payment" value="e" onclick="getpayment(this.value)">    <label style="font-weight: bold; color: green; font-size: 20px;" >ESewa</label> <br>
<!-- <input style="width: 25px; height: 25px;" type="radio" name="payment" value="k" onclick="getpayment(this.value)">    <label style="font-weight: bold; color: green; font-size: 20px;">Khalti</label> <br> -->
<input style="width: 25px; height: 25px;" type="radio" name="payment" value="b" onclick="getpayment(this.value)">    <label style="font-weight: bold; color: green; font-size: 20px;">Bank</label> <br>
<!-- <input style="width: 25px; height: 25px;" type="radio" name="payment" value="i" onclick="getpayment(this.value)">    <label style="font-weight: bold; color: green; font-size: 20px;">IME</label> <br></div>
...
<div id="demo" class="">
<button  class="w3-circle w3-hover-blue " style="background-color: orange; font-weight: bold; width: 40%; height: 8vh; color: white;" value="Conform Checkout" type="submit" name="cash"> Proceed Checkout</button><a href="#shipp"><button type="button" class="w3-circle w3-hover-blue " style="background-color: pink; font-weight: bold; width: 60%; height: 8vh; color: red;" > Change Shipping Address</button></a>
</div>
...
function getpayment(int) {
  ...
  xmlhttp.open("GET","payment_method.php?q="+int,true);
  xmlhttp.send();
}
</script>
...
<!-- <form  action="order_update.php" method="POST"  enctype="multipart/form-data"> -->
	<!-- <div id="demo" class="">
<button  class="w3-circle w3-hover-blue " style="background-color: orange; font-weight: bold; width: 40%; height: 8vh; color: white;" value="Conform Checkout" type="submit" name="submit"> Proceed Checkout</button><a href="#shipp"><button type="button" class="w3-circle w3-hover-blue " style="background-color: pink; font-weight: bold; width: 60%; height: 8vh; color: red;" > Change Shipping Address</button></a> -->
...
<input class="form-control" type="hidden"  required  value=" <?php echo htmlentities($result['cname']) ?>" name="cname">
...
<input type="hidden" name="country" class="form-control  " required  value=" <?php echo($result['country']) ?>" class=" ml-4 mr-1" >
