...
<br>
 ...
 <h2 class="w3-center"><b style="color: orange;"> Your Order List </b></h2>
<hr>
...
<table class="container-fluid w3-rose responsive table-stripe w3 w3-border responsive" border="2" cellpadding="10"  cellspacing="10">
    ...
    <th class="w3-center" colspan="2">pay_method</th>
...
$cc=$_SESSION['id'];
            ...
            // $id=$_GET['$cc'];
            $selectquery = "SELECT*FROM `order` WHERE cid ='$cc' AND subtotal=0  order by id desc";
...
$total+=$price;
  ...
  ?>
	<tr>
            ...
            <?php if ($status==='Proceeding') {?>
              <a href="order_cancel.php?id=<?php echo $result['id']; ?> "  data-toggle="tooltip" data-placement="bottom" title="Cancel" onClick="return confirm('Are you sure you want to remove yur order Item,note: We are not responsible after remove from you.');"><span style="font-weight: bold;font-size: 19px; color:red;">Cancel </span> </a>

             <?php
            } else{?>

  <a href="order_delet.php?id=<?php echo $result['id']; ?> "  data-toggle="tooltip" data-placement="bottom" title="delete" onClick="return confirm('Are you sure you want to remove yur order Item,note: We are not responsible after remove from you.');"><i class="fa fa-trash w3-center fa-2x" style="color: red;" aria-hidden="true"></i> </a>
    ...
    <td class="w3-center"><b style="color: green;"><?php echo $result['cas']; ?></b></td>
     <td class="w3-center"><b style="color: green;">
<?php if ($file>'0' and $cas!='Cash on Delevery') {?>
 <span style="margin-right: 8px; color:indigo;">paid</span>
 <img src="<?php echo $result['file']; ?> " width="50px;" height="auto;"> 
 <?php
} if ($cas==='Cash on Delevery' and $status==='Handovered') {?>
  <span style="margin-right: 8px; color:blue; font-weight: bold;font-size: 16px;">Paid</span>

  <?php
}if ($cas==='Cash on Delevery' and $status==='Pending' ) {?>
  <span style="margin-right: 8px; color:blue; font-weight: bold;font-size: 16px;">cash on home</span>
  <?php

}if ($cas==='Cash on Delevery' and $status==='Proceeding' ) {?>
 <span style="margin-right: 8px; color:pink; font-weight: bold;font-size: 16px;">cash on home</span>
 <?php
}

if ($file<'0' and $cas!='Cash on Delevery') {?>
  <span style="margin-right: 8px; color:red; font-weight: bold;font-size: 20px;">unpaid</span>
  <?php
}
?>
      </b></td>
     <td class="w3-center"><b style="color: green;"><?php echo $result['status']; ?></b></td>
...
<table class="container-fluid "><tr><th class="w3-left w3-orange">delete cart</th></tr></table>
		...
		<table class="container-fluid w3-rose responsive table-stripe w3 w3- responsive" border="1" cellpadding="10"  cellspacing="10">
...
<!--    <td><b class="w3-left ml-3">product ID:</b></td>
  ...
  <tr>
   ...
   <td rowspan="2"><b class="w3-left ml-3">Payment Method:</b></td>
    <td class="w3-righ mr-3"><?php  echo $resulte['cas']; ?></td>
   
  </tr>
  <tr>
  
    <td class="w3-righ mr-3"><b>
              <?php if ($filee>'0' and $cass!='Cash on Delevery') {?>
 <span style="margin-right: 8px; color:indigo;">paid</span>
 <img src="<?php echo $resulte['file']; ?> " width="50px;" height="auto;"> 
 <?php
} if ($cass==='Cash on Delevery' and $stats==='Handovered') {?>
  <span style="margin-right: 8px; color:blue; font-weight: bold;font-size: 16px;">Paid</span>

  <?php
}if ($cass==='Cash on Delevery' and $stats==='Pending' ) {?>
  <span style="margin-right: 8px; color:blue; font-weight: bold;font-size: 16px;">cash on home</span>
  <?php

}if ($cass==='Cash on Delevery' and $stats==='Proceeding' ) {?>
 <span style="margin-right: 8px; color:pink; font-weight: bold;font-size: 16px;">cash on home</span>
 <?php
}

if ($filee<'0' and $cass!='Cash on Delevery') {?>
  <span style="margin-right: 8px; color:red; font-weight: bold;font-size: 16px;">unpaid</span>
  <?php
}
?>
      </b></td>
...
<tr>
    <td><b class="w3-left ml-3">status:</b></td>
    <td class="w3-righ mr-3" style="color: green; font-weight: bold;"><?php echo "$stats"; ?></td>
  </tr>
      <tr style="border-bottom:  2px solid gray; margin-top:  8px;" ><td colspan="2">  <button class="form-control"><?php if ($stats==='Proceeding') {?>
              <a href="order_cancel.php?id=<?php echo $result['id']; ?> "  data-toggle="tooltip" data-placement="bottom" title="Cancel" onClick="return confirm('Are you sure you want to remove yur order Item,note: We are not responsible after remove from you.');"><span style="font-weight: bold;font-size: 19px; color:red;"><button class="w3-circle" style="width: 30%;"><i style="font-weight: bold; font-size: 28px; color: black;">Cancel</i></button> </span> </a>

             <?php
            } else{?>

  <a href="order_delet.php?id=<?php echo $result['id']; ?> "  data-toggle="tooltip" data-placement="bottom" title="delete" onClick="return confirm('Are you sure you want to remove yur order Item,note: We are not responsible after remove from you.');"><i class="fa fa-trash w3-center fa-2x" style="color: red;" aria-hidden="true">Remove</i> </a>
...
<!-- <form  action="order_update.php" method="POST"  enctype="multipart/form-data"> -->
	<div id="demo" class="">
<!-- <button  class="w3-circle w3-hover-blue " style="background-color: orange; font-weight: bold; width: 40%; height: 5vh; color: white;" value="Conform Checkout" type="submit" name="submit"> Proceed Checkout</button> -->
<a href="#shipp"><button type="button" class="w3-circle w3-hover-blue " style="background-color: pink; font-weight: bold; width: 90%; height: 5vh; color: red;" > Change Shipping Address</button></a>
...
?>
  ...
  <hr>
	...
	<br><br><br><input class="form-control container w3-orange w3-hover-blue" type="submit" name="oorder" required  value="Submit">
...
$ids=$_POST['pid'];
 // echo "$ids";
  ...
  $sql=" UPDATE `order` SET `cname`='$cname', `phone`='$phone',`country`='$country',`state`='$state',`district`='$district',`city`='$city',`localaddress1`='$localaddress1',`local_ad`='$local_ad',`zipcode`='$zipcode'  WHERE cid='$ids' and subtotal='0' and status='pending'";
...
echo  "<script>alert('Your Delevery Address Updated successfully !. Thankyou for joining us @ kupindetimur Team..');</script>";
