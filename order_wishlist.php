...
$cc=$_SESSION['id'];
            ...
            // $id=$_GET['$cc'];
            $selectquery = "SELECT*FROM `order` WHERE cid ='$cc' AND subtotal=2 ";
...
// echo $munci;
            ...
            $select = " SELECT * FROM `warda_munci` WHERE  warda='$tunit' and warda='tunit'";
...
<form  action="" method="POST"  enctype="multipart/form-data">
  <div id="demo" class="">
<button  class="w3-circle w3-hover-blue " style="background-color: orange; font-weight: bold; width: 40%; height: 5vh; color: white;" value="Conform Checkout" type="submit" name="wish"> Proceed All Checkout</button><a href="#shipp"><a href="#shipp"><button type="button" class="w3-circle w3-hover-blue " style="background-color: pink; font-weight: bold; width: 60%; height: 5vh; color: red;" > Change Shipping Address</button></a>
...
if($data)
 {
 // echo "<script type='text/javascript'> document.location = 'order_checkout.php'; </script>";


  echo "<script>alert('your order Conformed  successfully.please check order and kart section for detail. thanks for being kupindetimur family @kupindetimur.com Team');</script>";
...
$ids=$_POST['pid'];
 // echo "$ids";
  ...
  $sql=" UPDATE `order` SET `cname`='$cname', `phone`='$phone',`country`='$country',`state`='$state',`district`='$district',`city`='$city',`localaddress1`='$localaddress1',`local_ad`='$local_ad',`zipcode`='$zipcode',`subtotal`='0' ,status='pending' WHERE cid='$ids' and subtotal='2'";
...
echo  "<script>alert('Your Order request Submitted successfully,please check email to conform');</script>";
header('locaction:order_process.php');
}else{
